...
<script lang="ts">
  ...
  import {
    ...
    type Match,
    type MatchSide,
    type PingPongDataSource,
    type Player,
    type Session
  } from '$lib/ping-pong';

  type BoardMode = 'local' | 'session';

  let { mode = 'local', sessionCode = '' } = $props<{
    mode?: BoardMode;
    sessionCode?: string;
  }>();
  ...
  let dataSource = $state<PingPongDataSource | null>(null);
  let hasLegacyLocalData = $state(false);
  ...
  function hasAnyStateData(state: { players: Player[]; matches: Match[] }) {
    return state.players.length > 0 || state.matches.length > 0;
  }

  function getLegacyImportKey(code: string) {
    return `${LEGACY_IMPORT_STORAGE_PREFIX}:${code}`;
  }

  function getShareUrl() {
    if (typeof window === 'undefined' || !activeSession) return '';
    return `${window.location.origin}/s/${activeSession.code}`;
  }
  ...
  async function copyShareLink() {
    ...
    alert('Link do sesji skopiowany.');
  }

  async function reloadSessionState(source = dataSource, code = sessionCode) {
    if (!source || mode !== 'session') return;

    const state = await source.loadSession(code);
    skipNextSave = true;
  ...
  async function importLegacyLocalData() {
    if (!activeSession || !dataSource) return;

    const legacyState = loadPingPongState(localStorage, STORAGE_KEY);

    if (!hasAnyStateData(legacyState)) {
    ...
    const shouldImport =
      players.length === 0 && matches.length === 0
        ? true
        : confirm('Ta sesja ma już dane. Import lokalnych danych zastąpi aktualną zawartość sesji. Kontynuować?');

    if (!shouldImport) return;

    players = legacyState.players;
    matches = legacyState.matches;
    await dataSource.saveSessionState({
  ...
  onMount(() => {
    ...
    async function loadState() {
      ...
      try {
        const legacyState = loadPingPongState(localStorage, STORAGE_KEY);
        hasLegacyLocalData =
          hasAnyStateData(legacyState) &&
          (mode !== 'session' || localStorage.getItem(getLegacyImportKey(sessionCode)) !== '1');

        if (mode === 'local') {
          players = legacyState.players;
          matches = legacyState.matches;
        } else {
          const source = createPingPongDataSource();
          dataSource = source;
          await reloadSessionState(source, sessionCode);

          if (activeSession) {
            unsubscribe = source.subscribeToSession(activeSession, () => {
              void reloadSessionState(source, sessionCode);
            });
          }
        }
        ...
        loadError = 'Nie udało się wczytać danych sesji.';
      } finally {
        isLoading = false;
      }
    }
  ...
  $effect(() => {
    if (!hasLoadedState || isLoading) return;
    ...
    if (activeSession && dataSource) {
      ...
      void dataSource
        ...
        .catch((error) => {
          console.error('Nie udało się zapisać danych sesji:', error);
          loadError = 'Nie udało się zapisać danych sesji.';
        })
        .finally(() => {
          isSaving = false;
        });
...
<div class="app">
  <header>
    <div>
      ...
      <p>
        {#if mode === 'session' && activeSession}
          Sesja {activeSession.code}. Dodawaj zawodników, rozgrywaj mecze i dziel ranking ze znajomymi.
        {:else}
          Lokalny tryb gry. Dodawaj zawodników, rozgrywaj mecze na żywo i śledź ranking ELO.
        {/if}
      </p>
    </div>
    ...
    {#if mode === 'session' && activeSession}
      <div class="session-actions">
        <span class="session-badge">{dataSource?.name === 'supabase' ? 'Supabase' : 'Lokalna sesja'}</span>
        <button class="secondary" onclick={copyShareLink}>Kopiuj link</button>
      </div>
    {/if}
  </header>
  ...
  {#if isLoading}
    <section class="card">
      <p class="muted">Wczytywanie danych...</p>
    </section>
  {:else if loadError}
    <section class="card">
  ...
  {#if mode === 'session' && !isLoading && hasLegacyLocalData}
    <section class="card import-card">
      <div>
        <h2>Masz lokalne dane</h2>
        <p class="muted">Możesz ręcznie przenieść graczy i historię ze starego trybu do tej sesji.</p>
      </div>
      <button class="secondary" onclick={importLegacyLocalData}>Importuj do sesji</button>
    </section>
  {/if}

  {#if mode === 'session' && isSaving}
    <p class="save-status">Zapisywanie zmian...</p>
  {/if}
